...
// const HANDLE_RE = /(?<!\w)@[A-Za-z0-9._\-]+/g;
...
//       normal:           SECTION_SCHEMA,
//       more_info:        SECTION_SCHEMA
//     },
//     required: ['has_captcha', 'rejection_reason', 'normal', 'more_info']
//   }
// };
...
//   'Otherwise: split parsed text into "normal" (above the “More info” heading) and "more_info" (under it). ' +
//   'For each section include: emails (literal addresses), handles (strings starting with "@"), fields (array of {key,value}), raw_text (concise). ' +
//   'Return JSON only.';

// const USER_INSTRUCTIONS =
//   'Return only JSON. If "More info" is absent, put everything in "normal" and leave "more_info" empty.';
...
//   return ['has_captcha', 'rejection_reason', 'normal', 'more_info'].every(k => k in result);
// }
...
// // ---------- Post-processing to PRECISE shape you requested ----------
// function extractYouTube(fieldsArray = [], raw = '') {
//   // Prefer fields array with key "YouTube"
...
// function onlyEmailsHandles(section) {
//   const emails  = uniqueSorted(section?.emails  || []);
//   const handles = uniqueSorted(section?.handles || []);
...
//   const normal = onlyEmailsHandles(parsed?.normal);
...
//   return { has_captcha, normal, more_info };
// }

// // ---------- Single ----------
// async function extractEmailsAndHandles(req, res) {
//   try {
//     if (!OPENAI_API_KEY) throw new Error('Missing OPENAI_API_KEY');

//     // Build imagePart from one source (file/url/path)
//     let imagePart = null;

//     if (req.file && req.file.buffer) {
//       imagePart = await imagePartFromBuffer(req.file.buffer, req.file.mimetype);
//     } else if (req.body?.imagePath) {
//       imagePart = await imagePartFromPath(path.resolve(String(req.body.imagePath)));
//     } else if (req.body?.imageUrl) {
//       imagePart = imagePartFromUrl(req.body.imageUrl);
//     } else {
//       throw new Error('No image provided. Use multipart field, or body.imageUrl, or body.imagePath.');
//     }

//     const cacheKey = hashString(`s1|${JSON.stringify(imagePart.image_url)}|${MODEL_PRIMARY}|${MODEL_FALLBACK}|${PRIMARY_TOKENS}|${RETRY_TOKENS}|${IMAGE_DETAIL}`);
...
//   } catch (err) {
//     console.error('extractEmailsAndHandles error:', err);
//     return res.status(400).json({ status: 'error', message: err?.message || 'Failed to process image.' });
//   }
// }
...
//     // Accept up to 5 images via multipart, OR an array of imageUrl, OR array of imagePath
//     const out = [];
//     const tasks = [];
...
//           const cacheKey = hashString(`b1|${f.originalname}|${f.size}|${MODEL_PRIMARY}|${MODEL_FALLBACK}|${PRIMARY_TOKENS}|${RETRY_TOKENS}|${IMAGE_DETAIL}`);
//           const cached = cacheGet(cacheKey);
...
//             const cacheKey = hashString(`b2|${u}|${MODEL_PRIMARY}|${MODEL_FALLBACK}|${PRIMARY_TOKENS}|${RETRY_TOKENS}|${IMAGE_DETAIL}`);
//             const cached = cacheGet(cacheKey);
//             if (cached) return cached;
//             const parsed = await callVisionFast(imagePart);
//             const payload = shapeForClient(parsed);
...
//             const cacheKey = hashString(`b3|${pth}|${MODEL_PRIMARY}|${MODEL_FALLBACK}|${PRIMARY_TOKENS}|${RETRY_TOKENS}|${IMAGE_DETAIL}`);
//             const cached = cacheGet(cacheKey);
//             if (cached) return cached;
//             const parsed = await callVisionFast(imagePart);
//             const payload = shapeForClient(parsed);
