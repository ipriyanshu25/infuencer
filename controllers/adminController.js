...
exports.getByInfluencerId = async (req, res) => {
  try {
    ...
    if (!influencerId) {
      return res
        .status(400)
        .json({ message: 'Query parameter influencerId is required.' });
    }

    // 2) Find and project
    const influencer = await Influencer
      .findOne({ influencerId })
      .select('-password -__v')
      .lean();
...
exports.getCampaignsByBrandId = async (req, res) => {
    try {
        ...
        // 2) (Optional) pagination, search, sort & status
        const page = Math.max(parseInt(req.body.page, 10) || 1, 1);
        const limit = Math.min(Math.max(parseInt(req.body.limit, 10) || 10, 1), 100);
        const search = (req.body.search || '').trim();
        const sortBy = req.body.sortBy || 'createdAt';
        const sortOrder = (req.body.sortOrder || 'desc').toLowerCase();
        const statusFlag = parseInt(req.body.status, 10) || 0;  // 0=all,1=active,2=inactive
